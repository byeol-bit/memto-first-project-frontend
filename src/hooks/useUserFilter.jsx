...
import { useSearchUsers, useUsersData } from './queries/use-users-data';
...
const useUserFilter = () => {
  ...
  const isSearching = searchKeyword || tag.length > 0;

  const {data: users = [], isLoading, error } = 
    isSearching ?
    useSearchUsers({nickname: searchKeyword, category: tag})
    : useUsersData()
